/**
 * ===============================================
 * Document Loader, Embedding & PGVector API (Batch Processing)
 * ===============================================
 *
 * ฟีเจอร์หลัก:
 * - โหลดและประมวลผลเอกสารจากโฟลเดอร์ data/text_csv/
 * - แปลงเอกสารเป็น embeddings ด้วย OpenAI
 * - เก็บใน Supabase Vector Store (pgvector)
 * - รองรับไฟล์ .txt และ .csv (ไม่รองรับ PDF)
 * - Text splitting สำหรับ chunk ขนาดเหมาะสม
 * - รองรับการโหลดแบบ batch เพื่อหลีกเลี่ยง timeout
 *
 * API Endpoints:
 * - GET: โหลดเอกสารและสร้าง embeddings
 *   Query Parameters:
 *   - batch: 0, 1, 2 (ระบุ batch ที่ต้องการโหลด, default: โหลดทั้งหมด)
 *   - batchSize: จำนวนไฟล์ต่อ batch (default: 3)
...
/**
 * GET API: โหลดเอกสาร สร้าง embeddings และเก็บใน vector store (รองรับ batch processing)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (existingCount && existingCount > 0) {
      console.log(
        `🗑️ พบข้อมูลเก่า ${existingCount} records - ลบข้อมูลเก่าก่อน...`
      );
    ...
    if (processedDocs.length > 0) {
      ...
      const isPdf = (firstDoc.metadata.source || "")
        .toLowerCase()
        .endsWith(".pdf");
      console.log(
        `📋 ตัวอย่างข้อความ${
          isPdf ? "หลังแก้ไข" : ""
        } (200 ตัวอักษรแรก): ${preview}`
      );
    ...
    const chunksWithMetadata = chunks.map((chunk, index) => {
      ...
      const filename =
        source.split("/").pop() || source.split("\\").pop() || "unknown";
    ...
    const stats = {
      ...
      total_documents: rawDocs.length,
      total_chunks: chunks.length,
      files_processed: [...new Set(chunks.map((c) => c.metadata.source))].map(
        (source) => {
          const filename = source.split("/").pop() || source.split("\\").pop();
          const fileChunks = chunks.filter((c) => c.metadata.source === source);
          return {
    ...
    return NextResponse.json({
      message: `สำเร็จ! ${
        existingCount ? `ลบข้อมูลเก่า ${existingCount} records และ` : ""
      }สร้างและเก็บ ${chunks.length} chunks จาก ${rawDocs.length} เอกสาร`,
      stats,
      success: true,
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    const { error } = await supabase.from("documents").delete().neq("id", 0); // ลบทุกแถวที่ id ไม่เท่ากับ 0 (ซึ่งคือทุกแถว)
...
export async function PUT() {
  try {
    ...
    const fileStats =
      documents?.reduce((acc: Record<string, FileStats>, doc) => {
    ...
    console.log(
      `📊 สถิติข้อมูล: ${totalCount} records จาก ${
        Object.keys(fileStats).length
      } ไฟล์`
    );

    return NextResponse.json({
      message: `พบข้อมูล ${totalCount} records จาก ${
        Object.keys(fileStats).length
      } ไฟล์`,
      stats,
      success: true,
